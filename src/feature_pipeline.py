...
def run_feature_pipeline():
    ...
    data_output = args.output_data_path
    ...
    if config['mode'] == 'train':
        ...
        if config['features']['save_pipeline'] == True:
            ...
            save_model_to_path(full_pipeline, f'full_feature_pipeline', dir_path=model_output)

        logger.info(f'Processed data: {df_features.head()}')
        df_features.to_csv(f'{data_output}features_dataset.csv', index=False)
    logger.info('Pipeline completed.')
        ...
        #     save_model_to_path(rf_model, f'rf_model', dir_path=config['model']['dir_path'])
        #     if grid_model:
        #         save_model_to_path(grid_model, f'grid_search_model', dir_path=config['model']['dir_path'])
